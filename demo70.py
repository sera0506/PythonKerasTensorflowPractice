...
tbCallback = TensorBoard(log_dir="./logs/demo70", histogram_freq=0, write_graph=True, write_images=True)

model.fit(trainImages, trainLabels, epochs=20, callbacks=[tbCallback])
predictLabels = model.predict_classes(testImages)
print("predicted result=", predictLabels[:10])
